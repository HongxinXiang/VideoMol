...
def aug2_transforms_for_train(resize=224, mean_std=None, p=0.2, rotation=False):
    if mean_std is None:
        ...
        mean, std = mean_std[0], mean_std[1]
    img_transforms = transforms.Compose([transforms.CenterCrop(resize), transforms.RandomGrayscale(p),
                                         transforms.RandomRotation(degrees=360), transforms.RandomHorizontalFlip(),
